...
describe('MacDateRangePicker', () => {
  ...
  it('renders with default value', async () => {
    const el = await fixture<MacDateRangePicker>(
      html`<mac-date-range-picker .defaultValue=${['2024-06-01', '2024-06-15']}></mac-date-range-picker>`,
    )
  ...
  it('clears value when clear button clicked', async () => {
    const el = await fixture<MacDateRangePicker>(
      html`<mac-date-range-picker .defaultValue=${['2024-06-01', '2024-06-15']} clearable></mac-date-range-picker>`,
    )
  ...
  it('navigates months with prev/next buttons', async () => {
    ...
    const titles = el.shadowRoot!.querySelectorAll('.panel-header-title')
    const initialTexts = Array.from(titles).map((t) => t.textContent)

    const nextBtns = el.shadowRoot!.querySelectorAll('.panel-nav-btn')
    ;(nextBtns[1] as HTMLElement).click()
    await elementUpdated(el)

    const newTitles = el.shadowRoot!.querySelectorAll('.panel-header-title')
